/**
 * Default prompts (agents under test) seeded on first boot.
 *
 * A fresh install needs at least one prompt: a scenario cannot execute without a `promptId`, so
 * without these the quickstart dead-ends at "promptId must be a string".
 */
